...
var pokemonRepository = function () {
    ...
    $(window).on("keydown", function (event) {
        if (event.code === "Escape" && modalContainerEl.css("display") !== "none") hideModalBootstrap();
    });
    ...
    btnSearchPokemonEl.on("click", function () {
        var pokemonFound = searchPokemonByName(txtSearchPokemonEl.val().toLowerCase());
        if (pokemonFound) showDetails(pokemonFound);else showDetails();
    });
    ...
    function addPokemon(pokemon) {
        if ((typeof pokemon === "undefined" ? "undefined" : _typeof(pokemon)) === "object") {
            // Check If an The Object Contains All Keys of Pokemon Object
            validatePokemonObject(pokemon, pokemonObjKeys);
            pokemonList.push(pokemon);
        } else throw new TypeError("This function accepts only a 'pokemon' object as input!!!");
    }
    ...
    function addListItem(pokemon) {
        ...
        if (pokemon.pokemonID % 2 === 0) pokemonLiEl.addClass("pokemon-list-item__transform-1");else pokemonLiEl.addClass("pokemon-list-item__transform-2");
    ...
    function hideLoadingMessage(msg) {
        var status = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "successfully";
    ...
    function searchPokemonByName(pokemonName) {
        ...
        if (pokemonFound.length > 0) msg = "Pokemon " + pokemonName + " Exists. (n=" + pokemonFound.length + ")";else msg = "No Pokemon found with given Pokemon Name: " + pokemonName;
        // console.log(msg);
    ...
    function validatePokemonObject(obj, keyNames) {
        ...
        keyNames.forEach(function (keyName) {
            if (Object.keys(obj).some(function (key) {
                return key === keyName;
            }) === false) throw new TypeError("The input object must include all required data. Required keys are: " + pokemonObjKeys);
        });
    }
